...
// step length in clip coordinates 
//      (determine value using 1/(0.5 of kft / kftDelta))
var stepSize = 1.0/(0.5/kftDelta); 
// current y location change (init to no change)
var yUp = 0.0;
...
window.onload = function init()
{
	...
	yUpLoc = gl.getUniformLocation( program, "yUp" );
...
function render()
{
	...
	// set translation in y using stepSize in clip coordinates
	if(kft < 0.5) yUp = yUp + stepSize;
	else yUp = yUp - stepSize;
	// send updated value to GPU
    gl.uniform1f(yUpLoc, yUp);

	// we could use kft to set the rotation angle
