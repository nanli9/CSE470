...
window.onload = function init()
{
    ...
    for(var i=0;i<numCube-1;i++)
    {
        ...
        cubeCenter.push(vec3(0.5,0.5,-0.5));
...
function render()
{
    ...
    for(var z=0;z<numCube-1;z++){
        if(kft[z] == 0.0) {}
            ...
            angle[z]=kft[z]*stopAngle+(1-kft[z])*startAngle;
                ...
                for(var j=0;j<3;j++){
                    initPos[z][j] = initPos[z][j] + moveSpeed*rotateAxis[z][j];
                    cubeCenter[z][j] += moveSpeed*rotateAxis[z][j];
    ...
    for(var i=0;i<numCube-1;i++){
        ...
        mvMatrix = mult(mvMatrix,rotate(angle[i],rotateAxis[i][0],rotateAxis[i][1],rotateAxis[i][2]));
        gl.uniformMatrix4fv( modelView, false, flatten(mvMatrix) ); 
        gl.drawArrays( gl.TRIANGLES, 0, numTriangles*3,2);
